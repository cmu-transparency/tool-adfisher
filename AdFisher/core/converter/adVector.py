...
class AdVector:
    ...
    def display(self, choice):
        ...
        for ad in self.data:  # warning, ad module shadowed by loop variable
            i += 1
    ...
    def countLabels(self, lbl):
        c1 = 0
        for ad in self.data:  # warning, ad shadows module name
            if(ad.label == lbl):
                c1 += 1
        return c1
    ...
    def union(self, ads1):
        ...
        for ad in self.data:  # warning, ad shadows module name
            temp.add(ad)
        for ad in ads1.data:  # warning, ad shadows module name
            temp.add(ad)
        return temp.unique()
    ...
    def ad_weight(self, ad, wchoice):  # warning, ad shadows module name
        count = 0
        for a in self.data:
    ...
    def gen_word_vec(self, word_v, wchoice=W_CHOICE):
        # check generates a vector of words from AdVector, fits it to word_v
        vec = []
    ...
    def gen_ad_vec(self, ads, choice=CHOICE):
        # check self is ad_union
        # generates a vector of ads from AdVector
        vec = [0]*self.size()
        for ad in self.data:  # warning, ad shadows module name
            for lad in ads.data:
    ...
    def gen_temp_ad_vec(self, ads, choice=CHOICE):      # self is ad_union
        vec = [0]*ads.size()
        i = 0
        j = 0 # warning: not used

        for ad in ads.data:  # warning, ad shadows module name
    ...
    def gen_ad_words_vec(self, feat):
        # Creates a vector of word frequencies for each ad in AdVector
        vec = []
        for ad in self.data:  # warning, ad shadows module name
...
def ad_cosine_sim(ads1, ads2):  # check
    ...
    for ad in ad_union.data:  # warning, ad shadows module name
