...
class NewsVector:
    ...
    def gen_word_vec(self, word_v, wchoice=W_CHOICE):
        ...
        stemmed_words = common.stem_low_wvec(words) # warning: not used
        words = common.strip_vec(words)
        # print words
        for word in word_v:
            if(wchoice == NUM):
    ...
    def gen_news_vec(self, newsv, choice=CHOICE):
    ...
    def gen_temp_news_vec(self, newsv, choice=CHOICE):
        # self is news_union
        vec = [0]*newsv.size()
        i = 0
...
def news_cosine_sim(newsv1, newsv2):
    # check
    news_union = newsv1.union(newsv2)
    vec1 = []
    vec2 = []
