...
def stem_low_wvec(words):
    # check return stemmed and lower case words from the input list of words
...
def unique_words(words):
    # check returns a set of unique words from the input list of words
    unq = []
    for word in words:
        present = False
        for un in unq:
...
def strip_vec(list):
    # check removes the blank '', digits, $, & words
    try:
        if(list[0] == ''):
            del list[-len(list)]
        if(list[len(list)-1] == ''):
