...
class Ad:
    ...
    def contains(self, nonces):
        for nonce in nonces:
            if nonce in self.title.lower() or \
              nonce in self.url.lower() or \
              nonce in self.body.lower():
                # print self.label,
                # self.display()
                # raw_input("h")
                return True
    ...
    def fit_to_feat(self, word_v, wchoice):
        # fits an ad to a feature vector, returns a weight vector
        vec = []
        words = self.ad_to_words()
        stemmed_words = common.stem_low_wvec(words)  # warning: never used
