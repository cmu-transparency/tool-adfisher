...
def do_experiment(
    ...
    """Run an AdFisher experiment.

    Args:

      make_unit: Function that return a unit given a unit_id number (int). To keep the units
      exechangable, the unit_id should only be used as a label in logging.

      treatments: List of functions each of which that takes a unit and does some treatment to it.

      measurement: A function that takes a unit and makes some measurements of it. Should log the
      results.
      ...
      load_results: Load the results from a log file into a pair vectors:
        ...
        (2) one for the treatment assigned to each unit with the units in the same order.
      ...
      num_units: Number of units in each block. num_blocks * num_units is the sample size.

      timeout: How long to wait before giving up on a unit.

      log_file: File in which to store measurement and other helpful messages. Measurement should
      print to it.

      treatment_names: names of the treatments in the list treatments in the same order. Optionally
      makes logs look better.
    ...
    After doing this for every block, it uses test_stat to run a permutation test on the results
    using that as the test statistic.
    ...
    if analysis_flag:
        ...
        if(ml_analysis):
            ...
            classifier, observed_values, unit_assignments = analysis.ml.train_and_test(
                X, y,
                splittype='timed',
                splitfrac=0.2,
                nfolds=10,
            ...
            # topk0, topk1 = analysis.ml.print_only_top_features(
            # classifier, features, treatment_names, feat_choice="ads"
            # )
            # analysis.statistics.print_frequencies(X, y, features, topk0, topk1)

            print("Running permutation test\n")

            p_value = analysis.permutation_test.blocked_sampled_test(
                observed_values,
                unit_assignments,
