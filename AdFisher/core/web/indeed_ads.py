...
class IndeedAdsUnit(browser_unit.BrowserUnit):
    ...
    def __init__(self,
                 browser, log_file, unit_id, treatment_id,
    ...
    def create_user(self, gender, job_description):
        ...
        signin = self.driver.find_element_by_id("userOptionsLabel").click()  # warning: never used
        ...
        create_button = self.driver.find_element_by_css_selector(  # warning: never used
            '.input_submit.button.blueButton'
        ).click()
    ...
    def indeed_salary(self, fname, rel):
        ...
        sponsored_jobs = driver.find_element_by_css_selector(
            'div[data-tn-section="sponsoredJobs"]'
        )
        job_listings = sponsored_jobs.find_elements_by_css_selector(
            'div[data-tn-component="sponsoredJob"]'
        )
        ...
        for job in job_listings:
            ...
            if (len(salary) > 0):
                ...
                ad = strip_tags(
                    ctime+'@|'+salary_str+'@|'+'PLACEHOLDER'+'@|'+'PLACEHOLDER'
                ).encode("utf8")
    ...
    def indeed_ads(self, fname, rel, salary):
        ...
        for i in range(1, 4):
            ...
            ad = strip_tags(
                ctime+SEPARATOR+title.strip()+SEPARATOR+'URL'+SEPARATOR+location
            ).encode("utf8")
            self.log('measurement', 'ad', ad)
