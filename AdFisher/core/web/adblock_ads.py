...
class AdBlockUnit(browser_unit.BrowserUnit):
    ...
    def _fetch_easylist(self):
        '''Downloads the latest version of easylist, and if newer replaces any existing one.'''
    ...
    def _load_easylist(self):
        ...
        with open(self.EASYLIST) as f:
            lines = f.read().splitlines()
        print("Loaded easylist version: {} with : {} items"
              .format(self._easylist_version(),len(lines)))
        return lines
    ...
    def __init__(self,
        ...
        if rules is None:
            ...
            logging.basicConfig(filename="adb_"+log_file,
                                level=logging.INFO)
            ...
            self.Ad = namedtuple(
                'Ad',
                ['url', 'outerhtml', 'tag', 'link_text', 'link_location', 'on_site', 'reloads']
            )
    ...
    def save_data(self):
        ...
        self.logger.info(
            "save_data:{}:{}:{}".format(self.unit_id, self.treatment_id, self.session)
        )
    ...
    def check_elements(self, elements, source, options=None):
        ...
        Result: Queries the given attribute (source) and checks the url against the filterlist.
        Logs any identified elements and returns the count.
        '''
    ...
    def check_href(self):
        '''Identifies and captures ads based on HTML hyperlink tags. These are considered "text"
        ads.
        '''
    ...
    def check_src(self):
        '''Identifies and captures ads based on tags with a 'src' attribute These are considered
        "media" ads and are often img, iframe,script tags.
        '''
    ...
    def check_iframe(self, parents=()):
        '''Functionality to check within nested iframes for ad related resources. Invariants:
        expects webdriver to enter at the level defined by parents resets webdriver to top level
        contents prior to leaving Input: a tuple describing the iframe name attribute of parent
        levels.
        '''
