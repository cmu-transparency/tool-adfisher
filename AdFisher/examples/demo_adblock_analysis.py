...
def group_matrix(data):
    ...
    ads_by_url = group_on_url(all_data)  # warning: never used
...
def save_matrix_csv(data, groups, outfile):
    ...
    with open(outfile, 'wb') as csvfile:
        ...
        for g_id, g in groups.iteritems():
            ...
            link, urls = character
            csv_line = [
                g_id, list(link), list(urls), len(link), len(urls), sum(obs_data)
            ] + obs_data
            writer.writerow(csv_line)


def group_observed(obs, cols):
    def was_observed(c): return 1 if c in obs else 0
